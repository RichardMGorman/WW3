...
!>
!> @details Pre-processing of the input fields from multiple netcdf
!>  files, each corresponding to a different regular grid, to produce
!>  outputs on a quadtree mesh, either in ASCII format (for bathymetry
!>  and obstruction fields, or netcdf format (e.g. for wind, water
!>  levels, ...)
!>
!> @author R. Gorman
!> @date 15-Jun-2026
...
PROGRAM W3PRNQ
  ...
  !         June, 2026:       Add namelist input option
  !
  !/
  !  1. Purpose :
  !
  ...
  !     or latitude and longitude) on rank 2 [NX x NY] or rank 1 ([NX] 
  ...
  !     EXTIOF     Subr. W3SERVMD Abort when I/O file if error.
  !     EXTOPN     Subr. W3SERVMD Abort when opening file if error.
  ...
  USE W3SERVMD, ONLY: NEXTLN, EXTCDE, EXTIOF, EXTOPN
  USE W3NMLPRNQMD
#ifdef W3_S
  ...
  TYPE(NML_OUTVAR_COUNT_T)  :: NML_OUTVAR_COUNT
  TYPE(NML_OUTVAR_T), ALLOCATABLE  :: NML_OUTVAR(:)
  TYPE(NML_SUBGRID_COUNT_T)  :: NML_SUBGRID_COUNT
  TYPE(NML_SUBGRID_T), ALLOCATABLE  :: NML_SUBGRID(:)
  TYPE(NML_SUBGRID_VAR_T), ALLOCATABLE  :: NML_SUBGRID_VAR(:,:)
  INTEGER              :: NVAR, IVAR, ISG, ISG2, NVMAP
  INTEGER              :: LVLMXB, NINDBG
  INTEGER              :: LVLT, LVLMAX, LV0, LVLDIF
  INTEGER              :: LVLREF, LVLREFT, NSUBGRID
  INTEGER              :: NX, NY, NX0, NY0, NXP, NYP
  INTEGER              :: NXREF, NYREF
  INTEGER              :: OUTTYPE, UNDEF_TYPE, VALID_TYPE
  INTEGER              :: NCUT
  INTEGER              :: I, J, K, II, JJ, ICELL, IST
  INTEGER              :: NSEAMXB, NQMXB
  INTEGER              :: NCIDO
  INTEGER              :: NDSI, NDSA, NDSTRC, NTRACE
  INTEGER              :: ICLOSE
  ...
  !/
  !--- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ! 0.0  Set up grid storage structure
  !
  CALL W3NMOD ( 1, 6, 6 )
  CALL W3SETG ( 1, 6, 6 )
  CALL W3NOUT (    6, 6 )
  ...
  ! 1. User input
  !
  INQUIRE(FILE=TRIM(FNMPRE)//"ww3_prnq.nml", EXIST=FLGNML)
  IF (FLGNML) THEN
    ! Read namelist
    CALL W3NMLPRNQ (NDSI, TRIM(FNMPRE)//'ww3_prnq.nml', NML_GRID,  &
           NML_RECT, NML_FILE, NML_OUTVAR_COUNT, NML_OUTVAR,       &
           NML_SUBGRID_COUNT,NML_SUBGRID, NML_SUBGRID_VAR, IERR)
  ELSE
    OPEN (NDSI,FILE=TRIM(FNMPRE)//'ww3_prnq.inp',STATUS='OLD', IOSTAT=IERR)
    IF (IERR.NE.0) CALL EXTOPN(NDSE,IERR,'W3PRNQ','INPUT',60)
  END IF
  WRITE (NDSO,900)
  WRITE (NDSO,3000)
  !
  ...
  ELSE
    ...
    IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3PRNQ','INPUT',61)
    IF (COMSTR.EQ.' ') COMSTR = '$'
    WRITE (NDSO,901) COMSTR
    CALL NEXTLN ( COMSTR , NDSI , NDSE )
    READ (NDSI,*,IOSTAT=IERR) FLAGLL, CSTRG
    IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3PRNQ','INPUT',61)
  END IF
  !
  IF ( FLAGLL ) THEN
  ...
  !
  IF (FLGNML) THEN
    LVLREFT = NML_GRID%LVLREFT
    LVLMAX = NML_GRID%LVLMAX
  ELSE
    CALL NEXTLN ( COMSTR , NDSI , NDSE )
    READ (NDSI,*,IOSTAT=IERR) LVLREFT, LVLMAX
    IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3PRNQ','INPUT',61)
  END IF
  ...
  !
  IF (FLGNML) THEN
    OUTFILE = NML_FILE%FILENAME
  ELSE
    CALL NEXTLN ( COMSTR , NDSI , NDSE )
    READ (NDSI,*,IOSTAT=IERR) OUTFILE
    IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3PRNQ','INPUT',61)
  END IF
  ...
  !
  IF (FLGNML) THEN
    OUTTYPE = NML_FILE%OUTTYPE
  ELSE
    CALL NEXTLN ( COMSTR , NDSI , NDSE )
    READ (NDSI,*,IOSTAT=IERR) OUTTYPE
    IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3PRNQ','INPUT',61)
  END IF
  !
  ...
  !         
  IF (FLGNML) THEN
    NVAR = NML_OUTVAR_COUNT%NVAR
  ELSE
    CALL NEXTLN ( COMSTR , NDSI , NDSE )
    READ (NDSI,*,IOSTAT=IERR) NVAR
    IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3PRNQ','INPUT',61)
  END IF
  ...
  !     1.i.2 Quadtree quad file
    IF (FLGNML) THEN
      OUTFILEC = NML_FILE%OUTFILEC
      OUTFILEQ = NML_FILE%OUTFILEQ
    ELSE
      CALL NEXTLN ( COMSTR , NDSI , NDSE )
      READ (NDSI,*,IOSTAT=IERR) OUTFILEC
      IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3PRNQ','INPUT',61)
      CALL NEXTLN ( COMSTR , NDSI , NDSE )
      READ (NDSI,*,IOSTAT=IERR) OUTFILEQ
      IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3PRNQ','INPUT',61)
    END IF
  !
  ...
  ! Don't look for a time dimension or variable, just X&Y:
    RANK_XYT = 2
  ELSE     ! OUTTYPE options
    WRITE (NDSO,3009) 
  ! ... or output variables
    IF (FLGNML) THEN
      OUTVARNAME = NML_OUTVAR%OUTVARNAME
    ELSE
      DO IVAR=1,NVAR
        CALL NEXTLN ( COMSTR , NDSI , NDSE )
        READ (NDSI,*,IOSTAT=IERR) OUTVARNAME(IVAR)
        IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3PRNQ','INPUT',61)
      END DO
    END IF
  !
  ...
  ELSE
    CALL NEXTLN ( COMSTR , NDSI , NDSE )
    READ (NDSI,*,IOSTAT=IERR) NSUBGRID
    IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3PRNQ','INPUT',61)
  END IF
  ...
  !          Name of the netcdf file
    IF (FLGNML) THEN
      IPSG(ISG) = NML_SUBGRID(ISG)%IPSG
      FNAMES(ISG) = NML_SUBGRID(ISG)%FILENAME
    ELSE
      CALL NEXTLN ( COMSTR , NDSI , NDSE )
      READ (NDSI,*,IOSTAT=IERR) IDUM, IPSG(ISG), FNAMES(ISG)
      IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3PRNQ','INPUT',61)
    END IF
  ...
  !          variables in this file 
    DO K=1,RANK_XYT
      IF (FLGNML) THEN
        XYTDIMNAME(ISG,K) = NML_SUBGRID(ISG)%XYTDIMNAME(K)
        XYTVARNAME(ISG,K) = NML_SUBGRID(ISG)%XYTVARNAME(K)
      ELSE
        CALL NEXTLN ( COMSTR , NDSI , NDSE )
        READ (NDSI,*,IOSTAT=IERR) XYTDIMNAME(ISG,K)
        IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3PRNQ','INPUT',61)
        CALL NEXTLN ( COMSTR , NDSI , NDSE )
        READ (NDSI,*,IOSTAT=IERR) XYTVARNAME(ISG,K)
        IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3PRNQ','INPUT',61)
      END IF
  ...
  !          (ignored for OUTTYPE = 1)
      IF (FLGNML) THEN
        ITFIRST(ISG) = NML_SUBGRID(ISG)%ITFIRST
        ITLAST(ISG) = NML_SUBGRID(ISG)%ITLAST
        ITSTEP(ISG) = NML_SUBGRID(ISG)%ITSTEP
      ELSE
        CALL NEXTLN ( COMSTR , NDSI , NDSE )
        READ (NDSI,*,IOSTAT=IERR) ITFIRST(ISG),             &
  ...
  !        requested output variable
    DO IVAR=1,NVAR
      IF (FLGNML) THEN
        VARNAME(ISG,IVAR) = NML_SUBGRID_VAR(ISG,IVAR)%VARNAME
      ELSE
        CALL NEXTLN ( COMSTR , NDSI , NDSE )
        READ (NDSI,*,IOSTAT=IERR) VARNAME(ISG,IVAR)
        IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3PRNQ','INPUT',61)
      END IF
  ...
  ALLOCATE ( NXF(NSUBGRID), NYF(NSUBGRID), NTF(NSUBGRID), stat=IERR )
  ...
  !               NXYTVARID(ISG,1)
    IF ( NXYTVRANK(ISG,1).EQ.1 ) THEN
      ...
      DO J=2,NYF(ISG)
        XIN(:,J) = XIN(:,1)
      END DO
    ELSE IF ( NXYTVRANK(ISG,1).EQ.2 ) THEN
      ISTART2(1) = 1
      ISTART2(2) = 1
      ICOUNT2(1) = NXF(ISG)
      ICOUNT2(2) = NYF(ISG)
    ...
    IF ( NXYTVRANK(ISG,2).EQ.1 ) THEN
      ...
      DO I=2,NXF(ISG)
        YIN(I,:) = YIN(1,:)
      END DO
    ELSE IF ( NXYTVRANK(ISG,2).EQ.2 ) THEN
      ISTART2(1) = 1
      ISTART2(2) = 1
      ICOUNT2(1) = NXF(ISG)
      ICOUNT2(2) = NYF(ISG)
  ...
  !  coordinates:
    ...
    IF ( OUTTYPE.EQ. 1 ) THEN
      IF ( ALLOCATED(ARRIN) ) THEN
        ...
        IF ( IERR.NE.0 ) WRITE(NDSE,*) 'ARRIN dealloc. error',IERR
      END IF
    END IF
    IF ( ALLOCATED(ARMAP) ) THEN
      DEALLOCATE ( ARMAP, stat=IERR )
      IF ( IERR.NE.0 ) WRITE(NDSE,*) 'ARMAP dealloc. error',IERR
    END IF
...
subroutine gridint(M1,N1,X1,Y1,M2,N2,X2,Y2,Iint,Jint,Wint)
  ...
  !     left & rig ht walls: step if same side of both
  ...
  !                 opposite corner with vector from corner SS to corner TT
  !                 SS, TT = 00, 0p, p0, or pp
